...
#reactions
	...
	if($lOReact == 1 and $lin =~ /.*<celldesigner:baseReactant species.{2}(\w+).{2}alias.{2}(\w+)".+$/) {
		$ns[$rn]++;
		my $substrate = $1;
	...
	if($lOReact == 1 and $lin =~ /.*<celldesigner:reactantLink reactant.{2}(\w+).{2}alias.{2}(\w+)".+$/) {
		$ns[$rn]++;
                my $substrate = $1;
                $substrates[$rn][$ns[$rn]] = $substrate;
		my $salias = $2;
	...
	if($lOReact == 1 and $lin =~ /.*<celldesigner:baseProduct species.{2}(\w+).{2}alias.{2}(\w+)".+$/) {
		$np[$rn]++;
		my $product = $1;	
	...
	if($lOReact == 1 and $lin =~ /.*<celldesigner:productLink product.{2}(\w+).{2}alias.{2}(\w+)".+$/) {
		$np[$rn]++;
		my $product = $1;	
		$products[$rn][$np[$rn]] = $product;
		my $salias = $2;
